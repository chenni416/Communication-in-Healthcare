...
function PersonCard({ r, isFamilyView }: { r: HomeResident; isFamilyView?: boolean }) {
  const c = r.card;
  ...
  return (
    <Card headingLevel={2} variant={pendingFall ? "red" : "default"} title={<Link href={`/p/${r.patient_id}?tab=talk`} className="text-xl hover:text-primary">{r.code_name} <span className="text-sm font-normal text-ink-2">{r.room}</span></Link>} meta={c.latest_ts ? `最近 ${fmtDateTime(c.latest_ts)}` : undefined}>
      ...
      <Link href={`/p/${r.patient_id}?tab=talk`} className={`mt-3 flex items-center justify-center gap-1 rounded-[10px] px-4 font-medium ${pendingFall ? "min-h-16 text-lg bg-danger-ink text-on-primary" : "min-h-14 text-base bg-primary text-on-primary hover:bg-primary-hover"}`}>
        {/* {pendingFall ? "請確認他的狀況" : "講一句今天怎麼樣"} <ChevronRight className="size-5" aria-hidden="true" /> */}
        {pendingFall ? "立即確認狀況 (Urgent Check)" : "語音通報狀況 (Voice Report)"} <ChevronRight className="size-5" aria-hidden="true" />
      </Link>
      {/* 家屬版：補充觀察通道（建置中，後端第二階段實作） */}
      {isFamilyView && (
        <div className="mt-2 flex items-center gap-2">
          <button
            type="button"
            disabled
            title="功能規劃中：家屬補充的觀察將標記 family_reported，供護理師參考"
            className="flex-1 rounded-[10px] border border-dashed border-line py-2 text-sm text-ink-2 opacity-60 cursor-not-allowed"
          >
            補充觀察（建置中）
            📝 補充觀察 (Add Observation)
          </button>
          <TechTag label="規劃中" tech="family_reported 來源標記" scope="第二階段：家屬觀察寫入 Timeline，供護理師審核" />
        </div>
      )}
      {c.care_team && (
...
export default function CaregiverHome() {
  ...
  const sortedResidents = [...residents].sort((a, b) => {
    const aAlert = !!a.card.pending_event;
    const bAlert = !!b.card.pending_event;
    if (aAlert && !bAlert) return -1;
    if (!aAlert && bAlert) return 1;

    const aWarn = (a.card.changed_dimensions?.length ?? 0) > 0;
    const bWarn = (b.card.changed_dimensions?.length ?? 0) > 0;
  ...
  return (
    <div className="mx-auto max-w-[390px] space-y-4">
      ...
      <h1 className="text-balance text-2xl font-medium">{identity?.role === "family" ? "我的家人 (My Family)" : "照護名單 (Resident Roster)"}</h1>
      ...
      <ul className="grid gap-3">
        {sortedResidents.map((r) => (
          <li key={r.patient_id}><PersonCard r={r} isFamilyView={identity?.role === "family"} /></li>
        ))}
      </ul>
