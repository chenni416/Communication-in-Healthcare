  ...
  return (
    <li className="flex min-h-16 flex-wrap items-center gap-3 border-t border-line py-3">
      ...
      <div className="flex items-center gap-2">
        ...
        <Link href={`/p/${r.patient_id}?tab=docs`} className="inline-flex min-h-12 items-center rounded-[10px] bg-primary px-4 text-on-primary hover:bg-primary-hover">查閱報告 (View RoundPage) →</Link>
        <button
          type="button"
          disabled
          title="功能規劃中：醫師說完後觸發 Order Ingest Agent 並提案至護理站"
          className="inline-flex min-h-12 items-center gap-1 rounded-[10px] border border-dashed border-line px-3 text-sm text-ink-2 opacity-60 cursor-not-allowed"
        >
          🎙 口述醫囑
        </button>
        <TechTag label="規劃中" tech="STT → Order Ingest Agent" scope="第二階段：語音醫囑直接提案至護理站" />
      </div>
    </li>
  );
}
...
export default function DoctorHome() {
  ...
  return (
    <div className="space-y-3">
      ...
      <h1 className="text-2xl font-medium">今日巡診名單 (Today's Roster)</h1>
