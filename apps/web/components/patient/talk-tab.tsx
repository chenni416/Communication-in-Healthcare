    ...
    <div className={cn("flex flex-col gap-1", me ? "items-end" : "items-start")}>
      ...
      {!me && m.meta.activity && (
        <div className="flex flex-wrap items-center gap-2">
          <ActivityBar events={m.meta.activity} role={role} />
          {role !== "caregiver" && (
            <TechTag label="LangGraph" tech="LangGraph + GPT-5.6-luna" scope="Intake Agent 狀態機，追問上限 4 題" />
          )}
        </div>
      )}
    </div>
...
export function TalkTab({ summary, role, onChanged }: { summary: PatientSummary; role: Role; onChanged: () => void }) {
  ...
  const [justFinishedSpeech, setJustFinishedSpeech] = useState(false);
  ...
  const toggleListen = () => {
    ...
    rec.onend = () => {
      setListening(false);
      setJustFinishedSpeech(true);
      setTimeout(() => setJustFinishedSpeech(false), 2500);
    };
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100dvh-13rem)] w-full max-w-[390px] flex-col lg:min-h-[calc(100dvh-9rem)]">
      ...
      <form
        className="sticky bottom-14 flex flex-wrap items-end gap-2 border-t border-line bg-bg px-1 pt-2 pb-2 lg:bottom-0 lg:pb-[calc(0.5rem+env(safe-area-inset-bottom))]"
        onSubmit={(e) => {
          e.preventDefault();
          void send(input);
        }}
      >
        {justFinishedSpeech && (
          <p className="w-full text-[11px] text-warn-ink" aria-live="polite">
            ⚠ 請確認語音辨識是否正確，若有錯誤請修改後再送出
          </p>
        )}
        ...
        <input
          ...
          inputMode="text"
          enterKeyHint="send"
          className={`min-h-14 min-w-0 flex-1 rounded-[10px] border bg-bg px-4 text-base text-ink placeholder:text-ink-2 focus-visible:border-primary ${
            justFinishedSpeech ? "border-warn" : "border-line"
          }`}
        />
