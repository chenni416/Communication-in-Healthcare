    ...
    <div className="space-y-6">
      {role === "nurse" && pending.length > 0 && (
        <section aria-labelledby="pend" className="no-print space-y-4">
          ...
          <h2 id="pend" className="text-lg font-medium">AI 護理紀錄草稿 (Pending Confirmation)</h2>
          {pathA.map((p) => (
      ...
      {role !== "caregiver" && (
        <section aria-labelledby="rp">
          <div className="no-print mb-2 flex flex-wrap items-center gap-2">
            ...
            <TechTag label="Familiarization LLM" tech="GPT-5.6-luna + Trend Analyzer Agent" scope="從 timeline 與 baseline 生成，禁止診斷詞與 AI 建議" />
      ...
      {(role === "nurse" || role === "doctor" || role === "family") && incidents.length > 0 && (
        <section aria-labelledby="inc" className="no-print space-y-4">
          ...
          {incidents.map((d) => (
            <IncidentFileView key={d.id} d={d} codeName={name} role={role} />
          ))}
        </section>
      )}
