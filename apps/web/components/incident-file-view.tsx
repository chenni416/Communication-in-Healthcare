...
export function IncidentFileView({ d, codeName, role }: { d: IncidentFile; codeName: string; role?: string }) {
    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-2">
        ...
        {role !== "family" ? (
          ...
          <Card title={<div className="flex items-center gap-2">事件摘要 (白話文)<TechTag label="白話翻譯" tech="LLM 醫療術語白話文翻譯" scope="將專業 ISBAR 降維，消除家屬焦慮" /></div>} headingLevel={2}>
            <p className="text-base leading-relaxed">
              （模擬內容）護理站已初步了解狀況。{codeName} 剛才發生了些微異常，目前生命徵象穩定。護理師已親自過去關心並做完檢查，目前會持續觀察，請不用太擔心。
            </p>
          </Card>
        )}
      </div>
