...
export function AskBox({ pid, onSpeaking, onFocus }: { pid: string; onSpeaking?: (s: boolean) => void; onFocus?: (dimension: string | null) => void }) {
  ...
  return (
    <Card title="用說的問問題 (Voice Assistant)" headingLevel={2}>
      <form
      ...
      {ans && !busy && (
        <div className="ai-draft mt-3 p-3" aria-live="polite">
          <div className="mb-2 flex flex-wrap items-center gap-2">
              ...
              <TechTag label="AI 萃取" tech="RAG Vector Search + GPT-5.6-luna" scope="僅限個人 timeline 檔案，不猜測、不建議" />
            ...
            <TechTag label="TTS" tech="Web SpeechSynthesis API" scope="zh-TW 語音，本機瀏覽器執行" />
          </div>
          {ans.found ? (
